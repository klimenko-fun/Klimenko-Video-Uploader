package io.github.klimenko.videouploader.updater;

import io.github.klimenko.videouploader.utils.FileUtils;
import io.github.klimenko.videouploader.utils.HttpOperations;
...
import static io.github.klimenko.videouploader.utils.Constants.DATA_DIR;

/**
 * UpdaterCore contains the backend logic for update checking and "installing" of new updates. Its not really installing
 * because Klimenko Video Uploader does not register anything with the operating system, it keeps all its files in the
 ...
 * the signature from the update info retrieved from klimenko.fun when performing public key signature validation using
...
public class UpdaterCore {
            ...
            protected Boolean call() {
                try {
                    ...
                    acceptStatusFeed("Update is valid" + System.lineSeparator() +
                            "Preparing to update...");
                    String filename = "Klimenko_Video_Uploader-" + _info.getVersion().toString() + ".jar";
    ...
    private UpdateInfo fetchLatestVersionInfo() throws JSONException {
        String updateInformation = HttpOperations.getString("https://klimenko.fun/latestuploaderversion.json");
